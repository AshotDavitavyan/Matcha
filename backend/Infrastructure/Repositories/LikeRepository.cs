...
public class LikeRepository(DbConnectionFactory factory) : ILikeRepository
{
    public async Task LikeUser(int likerId, int likedId)
    {
        ...
        await using NpgsqlCommand likeUser = new NpgsqlCommand(
    ...
    public async Task UnlikeUser(int likerId, int likedId)
    {
        await using NpgsqlConnection conn = factory.CreateConnection();
        await conn.OpenAsync();
        await using NpgsqlCommand unlikeUser = new NpgsqlCommand(
            "DELETE FROM user_likes WHERE liker_id = @likerId AND liked_id = @likedId",
            conn);
    ...
    public async Task<bool> HasUserLiked(int likerId, int likedId)
    {
        await using NpgsqlConnection conn = factory.CreateConnection();
        await conn.OpenAsync();
        await using NpgsqlCommand query = new NpgsqlCommand(
            "SELECT EXISTS(" +
            "SELECT 1 FROM user_likes WHERE liker_id = @likerId AND liked_id = @likedId);",
            conn);
